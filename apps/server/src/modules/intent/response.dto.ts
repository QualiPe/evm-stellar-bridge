...
class LegEvmDto {
  ...
  @ApiPropertyOptional({
    example: '99500000',
    description: 'toAmount in minor units',
  })
  toAmountMinor?: string;
  @ApiPropertyOptional({
    example: '38.620321',
    description: 'toAmount in human units (optional)',
  })
  toAmount?: string;
...
class LegStellarDto {
  @ApiProperty({ example: 'strict-send' }) via!:
    | 'strict-send'
    | 'strict-receive';
  @ApiPropertyOptional({
    example: '124.80',
    description: 'strict-send: destination amount (human)',
  })
  destAmount?: string;
  @ApiPropertyOptional({
    example: '38.62',
    description: 'strict-receive: required source amount (human)',
  })
  sourceAmount?: string;
...
class MinLockDto {
  @ApiProperty({
    example: '99.50',
    description: 'EVM leg min lock (human, USDC)',
  })
  evm!: string;
  @ApiProperty({
    example: '99.50',
    description: 'Stellar leg min lock (human, USDC)',
  })
  stellar!: string;
}
...
class SummaryBridgeSideDto {
  ...
  @ApiPropertyOptional({
    example: '38576317',
    description: 'USDC minor amount (optional for EVM)',
  })
  minor?: string;

  @ApiProperty({
    example: 6,
    description: 'Decimals (6 for USDC on EVM, 7 on Stellar classical assets)',
  })
  decimals!: number;
}
...
export class IntentPlanDto {
  ...
  @ApiPropertyOptional({
    description: 'Desired output amount (human), when mode = EXACT_OUT',
    example: '100.0',
  })
  amountOut?: string;

  @ApiPropertyOptional({
    description: 'Estimated input amount (human), when mode = EXACT_OUT',
    example: '0.01',
  })
...
export class IntentRequestDto {
  ...
  @ApiPropertyOptional({
    description: 'Human amount when mode = EXACT_IN',
    example: '0.01',
  })
  amountIn?: string;

  @ApiPropertyOptional({
    description: 'Human amount when mode = EXACT_OUT',
    example: '25',
  })
  amountOut?: string;
